...
graph TD
    ...
    subgraph "Docker Builds (Staging - GHCR)"
        J_BUILD_DOCKER_VNC["build_docker_vnc\n(Build VNC Image)"]
        J_BUILD_DOCKER_MCP["build_docker_mcp\n(Build MCP Image)"]
    end

    subgraph "NPM GitHub Pipeline (Staging)"
        J_TEST_E2E["test_e2e\n(NPM E2E Tests)"]
        J_TEST_CONTAINERS["test_containers\n(NPM Container Tests)"]
        J_BUILD_GH_NPM["build_and_publish_github_npm\n(Build & Publish to GitHub NPM)"]
    end

    subgraph "NPMJS.org Pipeline (Production)"
        J_PREP_NPMJS["prepare_and_dry_run_npmjs\n(Prepare & Dry-Run NPMJS.org)"]
        J_PUB_NPMJS["publish_to_npmjs_org\n(Publish to NPMJS.org)"]
    end

    subgraph "Screenshots"
        J_SCREENSHOTS["take_screenshots\n(Generate & Publish Screenshots)"]
    end

    subgraph "Maintenance"
        J_CLEANUP["cleanup_old_versions\n(Cleanup Old Package Versions)"]
    end
